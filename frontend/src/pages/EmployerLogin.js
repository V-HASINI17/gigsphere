  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (loggedInUser.role !== "employer") {
        throw new Error("Invalid access. This login is for employers only.");
      }
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-brand-dark flex flex-col justify-center items-center p-6 transition-colors duration-300">
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mt-6 mb-8 space-y-2">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight">Employer Login</h2>
          <p className="text-xs text-slate-400">Login to post gigs and evaluate student candidates</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1.5">
            ...
            <input 
              ...
              className="w-full px-4 py-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-850 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-all"
            />
          </div>
        ...
        <div className="text-center mt-8 text-xs text-slate-400 font-medium">
          New Employer?{" "}
          <span onClick={() => navigate("/signup-employer")} className="text-indigo-550 hover:underline cursor-pointer">
